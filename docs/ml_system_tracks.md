...
| Track 2 | Kaggle | 100% | **Frozen** (KDR-001–KDR-009, 2026-06-28 to 2026-07-03; tag `track2-frozen`). K1–K5 complete; leakage-family decomposition complete; KDR-007 amended the objective (explain-the-gap → maximize private MCC) and closed P0 (raw-numeric signal probe, `H_raw_dominant` confirmed, public 0.39226/private 0.40391); KDR-008 closed P1 (station-temporal features + capacity tuning, `H_station_temporal_moderate` confirmed, program-best public 0.40447/private 0.41917); KDR-009 froze the track at the P1 result and revised the realistic target to ~0.435–0.445 (from the original ~0.52); a future KDR-010 "harvest-and-ensemble" probe is a possible, not authorized, reopening lead |
| Track 3 | Production | 100% | Frozen (tag: `track3-frozen`, commit: `f743da3`) |

**Current active initiative:** none of the three ML tracks — all are Frozen/100% as of `KDR-009`
(2026-07-03). Engineering effort has moved to the **portfolio transition**, a separate, non-ML-track
initiative governed by `docs/implementation/portfolio_master_plan.md` (phases PF0–PF8), which turns
this repository into a public engineering portfolio without reopening or redesigning any of the three
tracks above.

**Track 2 (Kaggle) closing summary** — K1 (honest baseline, public 0.14389/private 0.16160), K2 (record-adjacency magic, public 0.31699/private 0.32702), K3 (adjacency attribution: position-only public 0.31791/private 0.33161, label-only public 0.10065/private 0.10530 — neighbor-label leakage confirmed dead), K4 (label-free timing-cohort geometry: public 0.31697/private 0.33447, `H_cohort_modest`, record-order/timing family saturated), and K5 (duplicate-group identity attribution: label-free public 0.32330/private 0.33711; identity-conditioned label lookup public 0.33571/private 0.33989 — best model to date pre-P0, `H_duplicate_material` confirmed) are all complete, tagged (`K1-result`–`K5-result`), and merged to `main`. K1–K5 closed the leakage-family decomposition objective; KDR-007 then amended Track 2's objective to maximizing private-leaderboard MCC and pre-registered P0, a raw-numeric (968-column) signal probe layered on K5-A's honest feature stack. P0 is complete: regression anchor reproduced K5-A's OOF exactly (0.32506), Cell C (raw + K5-A stack) scored honest OOF 0.37598 (default capacity) / 0.37892 (high-capacity), both classifying `H_raw_dominant` (Δ ≥ +0.05 over K5-A); the high-capacity model's Kaggle submission scored public 0.39226 / private 0.40391, tagged `P0-result` and merged to `main`. KDR-008 then pre-registered and closed P1 (per-station timing Family D + station/line numeric aggregates Family S/L, layered on P0's Cell C stack, plus LightGBM `num_leaves`/`min_child_samples` tuning): the winning configuration (`num_leaves=255, min_child_samples=20`) scored honest OOF 0.39484 (Δ+0.01592 over P0), classifying `H_station_temporal_moderate`; its Kaggle submission scored public 0.40447 / private 0.41917 — the best result in the program, tagged `P1-result` and merged to `main`. P1's key finding: the new features contributed only +0.00306 OOF directly, while relieving LightGBM's leaf/capacity constraints on top of them recovered +0.01286 — capacity tuning, not feature engineering, was P1's dominant lever. KDR-009 (2026-07-03) then froze the track at this result: the original ~0.52 private-MCC target is not defended by the evidence gathered; the revised realistic range is ~0.435–0.445 via further tuning/blending, and P2 (categorical features) is deprioritized below that cheaper lever. Governance: `docs/research/kaggle_decisions.md` (`KDR-001`–`KDR-009`). Machine-readable ladder: `results/leaderboard.json`.
...
| Track 2: Kaggle Submission | Unlabeled Kaggle test in, `submission.csv` out | **Frozen** (`KDR-001`–`KDR-009`, 2026-06-28 to 2026-07-03; tag `track2-frozen`). K1 (honest baseline), K2 (record-adjacency magic), K3 (adjacency attribution), K4 (timing-cohort geometry), K5 (duplicate-identity attribution) complete, tagged, merged. `KDR-007` amended the objective to maximizing private LB MCC and closed P0 (raw-numeric signal probe, `H_raw_dominant`, public 0.39226/private 0.40391). `KDR-008` closed P1 (station-temporal features + capacity tuning, `H_station_temporal_moderate`), the program-best result (public 0.40447/private 0.41917). `KDR-009` froze the track at this result and revised the realistic target to ~0.435–0.445. |
...
4. **Phase 2 — Track 2 (Kaggle). Complete.** Track opened at `KDR-001` (2026-06-28); closed at
   `KDR-009` (2026-07-03, tag `track2-frozen`) after K1–P1 (nine sealed experiments, private MCC
   0.16160→0.41917). **Gate to Phase 3:** met — all three tracks Frozen/100%.
5. **Phase 3 — Portfolio transition. ← ACTIVE.** Governed by
   `docs/implementation/portfolio_master_plan.md` (PF0–PF8), not by this document — that ledger is
   the single execution plan for turning the repository into a public engineering portfolio. This
   file is not updated per-phase for Phase 3; see the master plan for status.
